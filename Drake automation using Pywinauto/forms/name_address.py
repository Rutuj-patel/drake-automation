...
def open_or_create_return(main_win, ssn: str, data: dict) -> None:
    ...
    popup_hwnd = wait_for_popup("Drake 2024 - Open Return", timeout=8)
    if not popup_hwnd:
...
def _find_name_form(timeout: int = 15) -> int | None:
    """
    Poll until a Drake Data Entry window appears with Name & Address controls.
    Accepts any Drake year (2023, 2024, 2025...).

    We distinguish the Name & Address form from the General tab by checking
    for controls that ONLY appear after navigation:
      15001 = Filing Status combo   (always on Name & Address)
      15050 = Street address edit   (always on Name & Address)
    The General tab only has controls 1001-1004 and 2001-2072 — never 15xxx.
    """
    deadline = time.time() + timeout
    print("[NAME] Waiting for Name & Address form", end="", flush=True)
    while time.time() < deadline:
        found = []

        def _cb(hwnd, _):
            title = win32gui.GetWindowText(hwnd)
            # "DRAKE 20" matches 2023, 2024, 2025 etc.
        ...
        for hwnd in found:
            ...
            if "15001" in hm and "15050" in hm:
                print(f"\n[NAME] Form window found (HWND: {hwnd})")
                return hwnd

        print(".", end="", flush=True)
        time.sleep(0.3)

    print("\n[NAME] Timeout — form not found")
    return None
...
def _handle_post_name_popup() -> None:
    ...
    # Try both years — VM may be 2023 or 2024
